...
def init_db(conn: sqlite3.Connection) -> None:
    ...
    conn.execute(
        ...
        values (?, ?, ?, ?)
        """,
        (UNIVERSE_ID, "A?????ETF?????", "active", created_at),
    )
    conn.execute(
        """
...
def build_strategy_data_check(
    ...
    if row_count < int(purpose_requirements["min_total_bars"]):
        missing_reasons.append(
            f"?K?????????? {purpose_requirements['min_total_bars']} ?????? {row_count} ??"
        )
    if post_warmup_bars < int(purpose_requirements["min_post_warmup_bars"]):
        missing_reasons.append(
            f"?????K???????? {purpose_requirements['min_post_warmup_bars']} ?????? {post_warmup_bars} ??"
        )
